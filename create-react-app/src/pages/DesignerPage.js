  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const apiParams = {
        ...
        size: parameters.基础设置?.大小?.split('(')[1]?.split('平方米')[0] || '',
      ...
      // 调用API生成图像
      const result = await generateTeaRoomImage(customPrompt, apiParams);
      
      console.log("API返回结果:", result); // 添加日志查看返回结果
      
      // 检查返回数据结构
      if (!result.data || !result.data[0]?.url) {
        throw new Error("API返回数据格式不正确，无法获取图像URL");
      }
      ...
      if (err.response) {
        ...
        // 请求发送但没有收到响应
        errorMessage = '无法连接到图像生成服务，请检查网络连接';
      } else {
        // 请求设置时出错
        errorMessage = err.message || '请求配置错误';
      }
      
      setError(`生成图像失败 ${errorCode ? `(错误码:${errorCode})` : ''}: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
